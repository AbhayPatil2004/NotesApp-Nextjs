...
import CodeCard from "@/components/MyUploadCodeCard";
    ...
    useEffect(() => {
        ...
        async function fetchCodes() {
            try {
                ...
                const res = await fetch("/api/myUploads/codes/abhay");

                // parse JSON once
                const payload = await res.json().catch(() => ({}));

                // debug log
                console.log("fetch /api/myUploads/codes/abhay -> status:", res.status, "payload:", payload);
                ...
                // accept various shapes including payload.body
                const dataArray =
                    payload?.body ??
                    payload?.fetchedData ??
                    payload?.codes ??
                ...
                if (mounted) {
                    setCodes(Array.isArray(dataArray) ? [...dataArray].reverse() : []);
                }
            } catch (err) {
